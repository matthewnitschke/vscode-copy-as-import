...
function activate(context) {
    const workspaceRoot = vscode.workspace.workspaceFolders && vscode.workspace.workspaceFolders.length > 0
        ...
        : undefined;
    let disposable = vscode.commands.registerCommand('copy-as-import.copy', (fileUri) => {
        if (!workspaceRoot) {
            vscode.window.showErrorMessage('Unable to find pubspec.yaml, cannot copy file as import.');
            return;
        }
        const pubspecPath = path.join(workspaceRoot, 'pubspec.yaml');
        const pubspecStr = (0, yaml_1.parse)(fs.readFileSync(pubspecPath, 'utf-8'));
        ...
        const packageName = pubspecStr.name;
